...
| 04 | StratLake Feature Series Index and Dual-Session Setup | [notebooks/04_stratlake_feature_series_index_setup.ipynb](../notebooks/04_stratlake_feature_series_index_setup.ipynb) | StratLake feature-series setup and dual-session Fintech/StratLake bridge; setup/import bridge, not feature generation | `fintech-market-ingestion` (upstream curated-data provider); `stratlake-trade-engine` (downstream session init) | `imported` | `cleaned`, `static_validated`, `readiness_validated`, `sanitized_execution_validated`, `cli_contract_validated`, `cli_registry_validated`, `audit_recorded`, `colab_smoke_pending` | [Notebook 04 import audit](notebook_04_import_audit.md); [staging/classification](notebook_04_staging_classification.md); [command surface classification](notebook_04_command_surface_classification.md) | `pending` | Issues #53–#56 imported, cleaned, classified command surfaces, added CLI contract/registry coverage, and added sanitized execution coverage. Notebook 04 introduces `FINTECH_SESSION_ID` / `STRATLAKE_SESSION_ID` dual-session pattern and the explicit `marketlake_root` handoff. Live Colab execution (package install, Drive mount, `fintech-init-project`, `stratlake-init-session`, Drive folder creation, archive/restore workflows) remains manual runtime-only. Repository validation is source-only and sanitized: no packages installed, no Drive mounted, no CLI commands executed, no archive/restore run, no feature data generated. Notebook 05 (`stratlake-init-session` feature generation) is future tutorial continuity only and is not implemented. |

Notebook 00, Notebook 01, Notebook 02, Notebook 03, and Notebook 04 are the imported notebooks currently tracked by this repository. Notebook 02 is repository-validated and audited, with Issue #37 manual Colab smoke recorded as `passed-with-notes`. Notebook 03 is repository-validated and audited as cleaned source with sanitized execution coverage, and Issue #51 records manual Colab smoke as `colab_smoke_passed_with_notes`. Notebook 04 is repository-validated and audited as cleaned source with sanitized execution coverage and CLI contract/registry coverage; manual Colab smoke remains `pending`. Repository validation does not run live Colab, Drive, package install, CLI, archive, restore, or feature-generation workflows for any of these notebooks.
