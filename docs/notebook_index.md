...
| 05 | StratLake Q1 Feature Data Generation with Fintech Daily Bars | [notebooks/05_stratlake_q1_feature_data_generation_with_daily_bars_ingestion.ipynb](../notebooks/05_stratlake_q1_feature_data_generation_with_daily_bars_ingestion.ipynb) | Q1 StratLake feature generation from Fintech curated daily bars | `fintech-market-ingestion` (upstream daily-bars ingestion); `stratlake-trade-engine` (downstream feature generation and dry-run export) | `imported` | `cleaned`, `static_validated`, `readiness_validated`, `sanitized_execution_validated`, `cli_contract_validated`, `cli_registry_validated`, `audit_recorded`, `colab_smoke_pending` | [Notebook 05 import audit](notebook_05_import_audit.md); [staging/classification](notebook_05_staging_classification.md); [command surface classification](notebook_05_command_surface_classification.md) | `pending` | Issues #61-#65 imported, cleaned, classified command surfaces, added CLI contract/registry coverage, added source-only readiness and sanitized execution coverage, and recorded audit/staging docs. Issue #66 did not perform a live Colab run in this repository session, so smoke remains pending. Notebook 05 initializes separate Fintech and StratLake sessions, pulls Q1 daily bars into local Fintech curated storage, passes that data to StratLake through `MARKETLAKE_ROOT`, builds Q1 daily features, and previews dry-run export/archive workflows. Repository validation is source-only and sanitized: it does not install packages, mount Drive, read Alpaca credentials, call Alpaca, run ingestion, run feature generation, export sessions, create archives, restore archives, or inspect generated runtime data. Runtime-only data needs include live Alpaca credentials/data access, `/content` workspaces, generated daily bars, generated features, and Google Drive session/archive storage. |

Notebook 00, Notebook 01, Notebook 02, Notebook 03, Notebook 04, and Notebook 05 are the imported notebooks currently tracked by this repository. Notebook 02 is repository-validated and audited, with Issue #37 manual Colab smoke recorded as `passed-with-notes`. Notebook 03 is repository-validated and audited as cleaned source with sanitized execution coverage, and Issue #51 records manual Colab smoke as `colab_smoke_passed_with_notes`. Notebook 04 is repository-validated and audited as cleaned source with sanitized execution coverage and CLI contract/registry coverage; Issue #59 records manual Colab smoke as `colab_smoke_passed_with_notes`. Notebook 05 is repository-validated and audited as cleaned source with CLI contract/registry coverage and sanitized execution coverage; manual Colab smoke remains `colab_smoke_pending`. Repository validation does not run live Colab, Drive, package install, credential, CLI, archive, restore, export, ingestion, or feature-generation workflows for any imported notebook.
