...
Notebook 00, Notebook 01, Notebook 02, Notebook 03, Notebook 04, and Notebook 05 are the imported notebooks currently tracked by this repository. Notebook 02 is repository-validated and audited, with Issue #37 manual Colab smoke recorded as `passed-with-notes`. Notebook 03 is repository-validated and audited as cleaned source with sanitized execution coverage, and Issue #51 records manual Colab smoke as `colab_smoke_passed_with_notes`. Notebook 04 is repository-validated and audited as cleaned source with sanitized execution coverage and CLI contract/registry coverage; Issue #59 records manual Colab smoke as `colab_smoke_passed_with_notes`. Notebook 05 is repository-validated and audited as cleaned source with CLI contract/registry coverage and sanitized execution coverage; Issue #66 records manual Colab smoke as `colab_smoke_passed_with_notes` from an uploaded executed artifact that is not committed. Repository validation does not run live Colab, Drive, package install, credential, CLI, archive, restore, export, ingestion, or feature-generation workflows for any imported notebook.

Notebook 02, Notebook 03, Notebook 04, and Notebook 05 preserve the runtime/archive boundary: active Colab app work remains under `/content`; Google Drive is archive/session storage only; Drive should not become the active app workspace; generated archive/session payloads, restored workspace files, restore outputs, archive packs, feature data, generated data, daily bars, notebook outputs, execution counts, credentials, private paths, and runtime folders stay out of Git.
